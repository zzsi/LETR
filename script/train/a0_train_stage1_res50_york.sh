...
if [ ! -d "$output"  ]; then
    ...
    # --master_port=$((1000 + RANDOM % 9999)) --nproc_per_node=1 --use_env  src/main.py --coco_path data/york_processed \
    ...
    PYTHONPATH=$PYTHONPATH:./src python -m src.main --coco_path data/york_processed \
    --output_dir $output --backbone resnet50 --resume  https://dl.fbaipublicfiles.com/detr/detr-r50-e632da11.pth \
    --batch_size 4 --epochs 200 --lr_drop 50 --num_queries 1000  --num_gpus 1  --layer1_num 3 | tee -a $output/history.txt
